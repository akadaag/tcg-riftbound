...
- Current milestone: `[x] M18 — Deep Upgrade Tree (Simplified)`
- App shell: `[x]`
- Authentication: `[x]`
- Cloud save sync: `[x]`
...
- Events/Missions: `[x]` (events system complete, player event planning with 6 event types, missions: 6 daily + 5 weekly + 19 milestone, seeded generation, day/week reset)
...
- Supplier System: `[~]` (M16 skipped — single instant-purchase supplier is sufficient; wholesale discounts from upgrades, staff, and events already provide depth)
- Polish/UI: `[x]`
...
- 2026-03-20 — **M18 complete (simplified).** Expanded upgrade system from 8 → 20 upgrades across 3 categories (Operations, Customer Experience, Business) with prerequisite chains. No specialization branches (simplified per user request for chill gameplay). 18.1: `UpgradeCategory` changed from 8 micro-categories to 3 (`operations`, `customer_experience`, `business`); 3 new `UpgradeEffectType` values added (`passive_income`, `hype_decay_reduction`, `event_revenue_bonus`); `prerequisites?: string[]` added to `UpgradeDefinition`. 18.2: 12 new upgrades added to catalog — Operations: `bulk_ordering` (prereq: supplier_contacts, wholesale -5%/L), `warehouse_expansion` (prereq: stock_room, +100 capacity/L), `efficient_shelving` (prereq: extra_shelf, +1 shelf/L); Customer Experience: `social_media_presence` (prereq: better_signage, +10% traffic/L), `vip_membership` (prereq: loyalty_program, +5 rep/day/L), `curated_showcase` (prereq: display_case_expansion, +2 display/L), `comfort_amenities` (prereq: premium_lighting, +5% tolerance/L), `hype_management` (prereq: social_media_presence, -15% hype decay/L); Business: `passive_income_stream` (+25 G/day/L), `advanced_training` (prereq: training_manual, +15% XP/L), `event_sponsorship` (+15% event revenue/L), `dropshipping` (prereq: passive_income_stream, +50 G/day/L). All 8 original IDs preserved for backward compatibility. `UpgradeModifiers` extended with `passiveIncome`, `hypeDecayReduction`, `eventRevenueBonus`; `getUpgradeModifiers` handles all 11 effect types. 18.3: `arePrerequisitesMet()` added; `canPurchaseUpgrade` gains optional 5th param `ownedUpgrades` for prerequisite validation. 18.4: Store `purchaseUpgrade` passes `state.save.upgrades` as 5th arg. 18.5: `decayHype()` in hype.ts gains optional `reductionFactor` param (caps at 0.9); `advanceDay()` passes `upgradeMods.hypeDecayReduction` to `decayHype`, adds `upgradePassiveIncome` to softCurrency alongside area passive income, calculates `eventRevenueBonus` from completed player events scaled by modifier; `calculateOfflineProgress` includes upgrade passive income in offline earnings. 18.6: Upgrades UI rebuilt with 3 category tabs, per-category descriptions, prerequisite lock indicators (lock icon + "Requires: X" text), new bonus types in summary. 18.7: No save migration needed — `UpgradeState[]` format unchanged. 18.8: Typecheck + build clean. Files modified: `src/types/game.ts`, `src/features/upgrades/index.ts`, `src/features/engine/hype.ts`, `src/features/engine/day-cycle.ts`, `src/stores/game-store.ts`, `src/app/more/upgrades/page.tsx`.

- 2026-03-20 — **M17 complete.** 17.1–17.2: `PlayerEventType` union (6 types: tournament, launch_party, sidewalk_sale, vip_night, card_signing, pack_cracking_stream), `PlayerEventStatus`, `PlannedEvent`, `PlayerEventResult` types added to `types/game.ts`; `SaveGame` extended with `plannedEvents`, `totalPlayerEventsHosted`, `playerEventCooldowns`; `"host_player_events"` added to `MissionType`. 17.3–17.8: `src/features/engine/event-planner.ts` created (pure functions) — `EVENT_PLANNER_UNLOCK_LEVEL=4`, `PLAYER_EVENT_DEFINITIONS` (6 full definitions with cost/prep/duration/cooldown/requirements/modifiers), `checkEventRequirements()` (validates area built, staff role, gold, level, cooldown, no duplicate pending), `createPlannedEvent()`, `cancelPlannedEvent()`, `processPlannedEventsForDay()` (decrements prep countdown, injects `GameEvent` when prep=0, detects completion when injected event expires, sets cooldowns), `buildGameEvent()`, `calculateEventResult()`, `getAvailableEventDefinitions()`, `getActivePlannedEvents()`, `getCompletedPlannedEvents()`, `countCompletedPlayerEvents()`. 17.9: `src/app/more/events/page.tsx` built — locked gate for shopLevel<4, active/preparing events section with cancel button, available events list filtered by shop level with per-event requirements checklist (area/staff/gold), prep+duration info, confirm-before-plan flow, recent events summary (customers, rep, hype). 17.10: Random events system unchanged; player events inject `GameEvent` objects into the existing system so both coexist. 17.11: `advanceDay()` in `day-cycle.ts` calls `processPlannedEventsForDay` after cleanup, merges new `GameEvent`s into `allEvents`, applies `reputationReward` to daily rep gain, applies `hypeBoost` to all `SetHype.currentHype` entries, updates `plannedEvents`/`playerEventCooldowns`/`totalPlayerEventsHosted` on save, surfaces `playerEventsCompleted` in `EndDayResult`. Store gains `planEvent` (validates + deducts gold) and `cancelPlannedEvent` actions. `createInitialSave` gains M17 defaults. Legacy migration 3h added to `auth-provider.tsx`. Events entry added to More menu (unlockLevel=4). Three milestone missions added (`ms_events_1` 500G, `ms_events_5` 2000G, `ms_events_15` 5000G). `host_player_events` case added to `evaluateMissionProgress()`. Supplier wholesale discount bug fixed (now includes `staffEffects.wholesaleDiscount`). Barrel export added to `engine/index.ts`. `tsc --noEmit` + `next build` pass clean.

- 2026-03-20 — **M15 complete.** 15.1–15.2: `StaffRole`, `StaffMember`, `StaffCandidate`, `StaffEffects` types added to `types/game.ts`; `SaveGame` extended with `staff: StaffMember[]`, `staffCandidates: StaffCandidate[]`, `staffCandidatesRefreshedDay: number`. 15.3–15.4: `src/features/engine/staff.ts` created — pure functions: `STAFF_UNLOCK_LEVEL=3`, `getMaxStaffSlots()` (scales L1=1 → L20=10), `generateCandidates()` (3–5 deterministic daily candidates seeded by day×shopLevel, randomized roles, levels 1–3, role-scaled salary), `hireStaff()`, `fireStaff()`, `giveRaise()`, `calculatePayroll()`. 15.5–15.8: `applyDayToStaff()` applies end-of-day XP (10/day, halved if morale<50), morale (+5 paid / -10 unpaid / -2 decay), level-up check (max 5), morale-boost on level-up, salary auto-scales on level-up, removes members below morale=30 (quit). `getStaffEffects()` aggregates per-role bonuses — greeter: +10% traffic/level, cashier: +5% tolerance/level, security: +4% tolerance + 0.5 rep/level, stocker: +0.5 rep/level, buyer: +3% wholesale discount/level, specialist: +8% competitive draw/level. `refreshCandidatesIfNeeded()` regenerates pool when day changes. 15.9: `src/app/more/staff/page.tsx` built — locked gate for shopLevel<3, active bonuses panel, staff roster with morale bar + XP bar + salary + fire/raise(+10G/+20 morale) buttons, hiring pool with candidate cards + hire button, refresh button, slot full warning. 15.10–15.11: Staff slot scaling and name pool (30 names) implemented in engine. 15.12: `advanceDay()` in `day-cycle.ts` deducts payroll (if affordable), calls `applyDayToStaff`, refreshes candidate pool for next day. `useSimulation.ts` computes `getStaffEffects` and stacks `trafficBonus`/`toleranceBonus`/`competitiveCustomerBonus` into `processTick`. Store gains `hireStaffMember`, `fireStaffMember`, `giveStaffRaise`, `refreshStaffCandidates` actions. `createInitialSave` gains staff defaults. Legacy migration 3g added to `auth-provider.tsx`. Staff entry added to More menu (unlockLevel=3). `tsc --noEmit` + `next build` pass clean.

- 2026-03-20 — **M14 complete.** 14.1–14.2: `ShopAreaType`, `ShopArea`, `AreaEffects` types added to `types/game.ts`; `SaveGame` extended with `shopAreas: ShopArea[]`. 14.3: `src/features/engine/areas.ts` created — 8 area definitions (sales_floor, singles_counter, storage_room pre-built at tier 1; tournament_hall, lounge, premium_vault, workshop, online_store purchasable), `AREA_DEFINITIONS` constant, `getAreaDefinition`, `getAllAreaDefinitions`, `getAreaState`, `isAreaBuilt`, `getAreaTier`, `getVisibleAreas`, `canBuildArea`, `canUpgradeArea`, `getAreaEffects` (aggregates `extraShelfSlots`, `trafficBonus`, `toleranceBonus`, `reputationPerDay`, `passiveIncomePerDay`, `competitiveCustomerBonus`, `whaleCustomerBonus`, `remoteTraffic`, `extraSinglesSlots`, `inventoryCapacityBonus`), `createInitialShopAreas`. 14.4–14.11: All 8 areas modeled with 3–4 upgrade tiers, cumulative effects — Sales Floor adds shelf slots + traffic, Singles Counter adds singles slots + competitive traffic, Storage Room adds inventory capacity, Tournament Hall boosts competitive/whale traffic + rep/day, Lounge adds passive income + casual traffic + tolerance, Premium Vault adds singles slots + whale traffic, Workshop adds passive income + singles slots + rep/day, Online Store adds remoteTraffic. 14.12: `src/app/shop/areas/page.tsx` built — lists all visible areas, shows current tier, next tier preview with cost, build/upgrade buttons with disabled states and reason text; active bonuses summary panel. 14.13: `processTick()` in `simulation.ts` gains `competitiveCustomerBonus`/`whaleCustomerBonus` params applied to `effectiveBonuses`; `useSimulation.ts` computes `getAreaEffects` and passes stacked bonuses into `processTick`. `advanceDay()` in `day-cycle.ts` applies area `reputationPerDay` on top of upgrade rep; adds `passiveIncomePerDay` to `softCurrency`. `calculateOfflineProgress()` applies proportional passive income scaled by `daysSimulated`. 14.14: Shop page gains "Shop Areas" link card. 14.15: `game-store.ts` — `buildArea`/`upgradeArea` actions implemented; `listSingle` slot cap dynamically reads `5 + areaEffects.extraSinglesSlots`; `buyFromSupplier` capacity check adds `areaEffects.inventoryCapacityBonus`; `createInitialSave()` includes `shopAreas`; legacy save migration in `auth-provider.tsx` step 3f. Engine barrel updated (`index.ts`). `tsc --noEmit` + `next build` pass clean.

- 2026-03-20 — **M13 complete.** 13.1: `DayPhase` type (`morning|afternoon|evening|night`), `PHASE_DURATION_MS` (180 000 ms), `DAY_DURATION_MS` (720 000 ms), `TICK_INTERVAL_MS` (1 000 ms), `ACTIVE_PHASES`, and `ShopNotification` added to types/game.ts; `SaveGame` extended with `lastTickAt`, `currentPhase`, `dayElapsedMs`; `OfflineReport` extended with `daysSimulated`. 13.2–13.3: `src/features/engine/simulation.ts` created — pure-function tick engine with `processTick()` (customer arrival scaled by phase traffic multiplier + phase type bias, shelf depletion, revenue, notifications), `simulateOfflineTicks()` (fast-forward N ticks at 60% efficiency across multiple days), and phase helpers (`getPhaseAtElapsed`, `getDayProgress`, `getPhaseStartMs`). Phase traffic multipliers: morning 0.65×, afternoon 1.3×, evening 1.05×, night 0×. Phase type biases skew customer mix per phase (morning → casual/kid; evening → collector/competitive/whale). 13.4–13.5: End Day button removed from `page.tsx`; day transitions fire automatically when simulation hook detects night→morning rollover; night phase shows daily summary panel with today's stats. 13.6: `calculateOfflineProgress()` in `day-cycle.ts` updated to call `simulateOfflineTicks`, sets `lastTickAt`/`currentPhase`/`dayElapsedMs` on returned save, passes `daysSimulated` in report. 13.7: Game store updated — M13 fields added to `createInitialSave()`, `notifications: ShopNotification[]` transient state, new actions `applyTickResult()`, `applyDayTransition()`, `addNotification()`, `clearNotifications()`. 13.8: Live shop dashboard built in `page.tsx` — phase indicator with animated progress bar, shelf stock gauges (green/yellow/red by stock level), scrollable notification feed, night summary panel, day progress %, phase dot row. 13.9–13.10: All 5 customer types and existing systems (hype decay, mission evaluation, random events, singles sales) preserved and wired through `advanceDay()` called automatically on each day transition. 13.11: Save migration in `auth-provider.tsx` — legacy saves without `lastTickAt`/`currentPhase`/`dayElapsedMs` get safe defaults on load. 13.12: `npx tsc --noEmit` + `next build` pass clean. New hook `src/hooks/useSimulation.ts` drives the 1 s tick loop from React (pauses when tab hidden). Files created: `src/features/engine/simulation.ts`, `src/hooks/useSimulation.ts`. Files modified: `src/types/game.ts`, `src/features/engine/day-cycle.ts`, `src/features/engine/index.ts`, `src/stores/game-store.ts`, `src/app/page.tsx`, `src/features/auth/auth-provider.tsx`.
